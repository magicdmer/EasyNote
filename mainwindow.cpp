...
void MainWindow::initNoteBook()
{
    ...
    foreach (const QString& groupName, groupDirs)
    {
        ...
        groupDir.setNameFilters(QStringList() << "*.txt");
    ...
    rootDir.setNameFilters(QStringList() << "*.txt");
    ...
    if (noNotes)
    {
        bool anyInGroups = false;
        foreach (const QString& g, groupDirs)
        {
            QDir gd(groupPath(m_notebook, g));
            gd.setNameFilters(QStringList() << "*.txt");
            if (!gd.entryList(QDir::Files).isEmpty()) { anyInGroups = true; break; }
        }
        noNotes = !anyInGroups;
    }
    ...
    else
    {
        ...
        else
        {
            ...
            foreach(const QString& entry, noteList)
            {
                ...
                if (slash >= 0) { group = entry.left(slash); name = entry.mid(slash+1); }
                else { name = entry; }

                QString filePath = noteFilePath(m_notebook, group, name);
                if (QFile::exists(filePath))
                {
                    NoteWidget* note = new NoteWidget(ui->tabWidgetNote,m_notebook,group,name,m_editor_font);
                    ui->tabWidgetNote->addTab(note,name);
                    if (entry == currentNote || name == currentNote)
                    {
                        curWidget = note;
                    }
                }
...
void MainWindow::newTab(const QString& groupName)
{
    for (int i = 1; i < 100; i++)
    {
        ...
        if (findNoteItem(groupName, newName) != nullptr) continue;
        if (QFile::exists(noteFilePath(m_notebook, groupName, newName))) continue;
        ...
        QTreeWidgetItem* item = addNoteItem(groupName, newName, now, now);
...
void MainWindow::sltListActonDelete()
{
    ...
    if (noteItem)
    {
        // tab 开着: 走 widget 的删除逻辑(以 textEdit 实时状态判定)
        delete item;
    ...
    else
    {
        // tab 没开: 只能以磁盘文件状态判定
        delete item;
        const QString notePath = noteFilePath(m_notebook, group, noteTitle);
        if (QFileInfo(notePath).size() == 0)
        {
...
void MainWindow::sltListMenuRequested(const QPoint &pos)
{
    ...
    foreach (const QString& g, groups) addGroupTarget(g, g);
